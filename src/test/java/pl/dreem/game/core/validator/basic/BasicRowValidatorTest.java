...
public class BasicRowValidatorTest {
    ...
    public void checkShouldFindWinPattern() {
        final BasicRowValidator sut = new BasicRowValidator();
        ...
        final Point secondPoint = Point.from(0, 1);
    ...
    public void checkShouldFindWinPatternRev() {
        ...
        final Point firstPoint = Point.from(0, 2);
        final Point secondPoint = Point.from(0, 1);
    ...
    public void checkShouldFindSuccessPattern() {
        ...
        final Symbol testSymbol = Symbol.O;
        final BoardCoordinates testLastCoordinates = BoardCoordinates.from(Point.from(2, 0));

        final Board testBasicBoard = Board.createBasicBoard();

        final Point firstPoint = Point.from(0, 0);
        final Point secondPoint = Point.from(1, 0);
        final Point thirdPoint = Point.from(2, 0);
